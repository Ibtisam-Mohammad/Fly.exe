...
def _environment(
    ...
    *,
    ...
    for role, means in (
        ...
        path.write_bytes(
            _mat_file({interval: _spread(mean, count, spread) for interval, mean in means.items()})
        )
        files[role] = path
    ...
    contract = {
        ...
        "depends_on": {"fit_artifact_sha256": sha256_file(fit_artifact)},
        "the_cohorts": {
            "primary": {
...
def test_the_contract_starts_unopened_and_names_only_control_arrays() -> None:
    contract = json.loads(CONTRACT.read_text(encoding="utf-8"))
