...
EON_CINEMATIC_STATUS: a 1080p presentation recorder and renderer are implemented but have not yet completed a clean-commit production run. The presentation must reproduce the accepted hero transition signature before rendering; it records full-graph spike counts and MuJoCo qpos, renders offline, and visibly distinguishes the 0.25 mm food object from the engineered 1.0 mm thorax-proximity sucrose zone
